...
export default function QuoteCard({ currentQuote, loading, phase, quoteIndex, lastCorrect, onAnswer }: Props) {
  ...
  const hint = () => {
    ...
    if (phase === 'answered' && displayQuote) {
      ...
      if (lastCorrect) {
        return wasReal ? '✓ correct — it was a real Kanye quote!' : '✓ correct — it was a fake quote!'
      } else {
        return wasReal ? '✗ wrong — it was actually a real Kanye quote!' : '✗ wrong — it was actually a fake quote!'
      }
    }
    return 'tap to reveal · next quote auto-loads'
  }

  const feedbackText = phase === 'answered'
  ...
  return (
    <div className="card card-quote" ref={cardRef}>
      ...
      {feedbackText && (
        <div className={`feedback-text ${lastCorrect ? 'correct' : 'wrong'} show`}>
          {feedbackText}
        </div>
      )}

      <div className="quote-tag">
        Quote #{quoteIndex} · Round 1
      </div>
